...
module('Unit | Model | new-platform-flexberry-g-i-s-map', function (hooks) {
  setupTest(hooks);
  ...
  test('it exists', function (assert) {
    const model = run(() => this.owner.lookup('service:store').createRecord('new-platform-flexberry-g-i-s-map'));

    assert.ok(!!model);
  });

  test('substitution _getModelLayerFeature', function (assert) {
    const map = run(() => this.owner.lookup('service:store').createRecord('new-platform-flexberry-g-i-s-map'));
  ...
  test('getMergedGeometry with difference should return geoJson feature in EPSG:4326', function (assert) {
    ...
    const feature1Layer1 = L.geoJSON(geoJson1Layer1).getLayers()[0];
    feature1Layer1.options.crs = { code: 'EPSG:4326', };
    const feature2Layer1 = L.geoJSON(geoJson2Layer1).getLayers()[0];
    feature2Layer1.options.crs = { code: 'EPSG:4326', };

    const feature1Layer2 = L.geoJSON(geoJson1Layer2).getLayers()[0];
    feature1Layer2.options.crs = { code: 'EPSG:4326', };
    const feature2Layer2 = L.geoJSON(geoJson2Layer2).getLayers()[0];
    feature2Layer2.options.crs = { code: 'EPSG:4326', };

    feature1Layer1.toJsts = function () { };

    const toJstsStub1 = sinon.stub(feature1Layer1, 'toJsts');
    const objJsts1 = geometryToJsts(geoJson1Layer1);
    objJsts1.setSRID(4326);
    toJstsStub1.returns(objJsts1);

    feature2Layer1.toJsts = function () { };

    const toJstsStub2 = sinon.stub(feature2Layer1, 'toJsts');
    const objJsts2 = geometryToJsts(geoJson2Layer1);
    objJsts2.setSRID(4326);
    toJstsStub2.returns(objJsts2);

    feature1Layer2.toJsts = function () { };

    const toJstsStub3 = sinon.stub(feature1Layer2, 'toJsts');
    const objJsts3 = geometryToJsts(geoJson1Layer2);
    objJsts3.setSRID(4326);
    toJstsStub3.returns(objJsts3);

    feature2Layer2.toJsts = function () { };

    const toJstsStub4 = sinon.stub(feature2Layer2, 'toJsts');
    const objJsts4 = geometryToJsts(geoJson2Layer2);
    ...
    const result = map.getMergedGeometry('1', ['1', '2'], '2', ['1', '2']);
  ...
  test('getMergedGeometry with union should return geoJson feature in EPSG:4326', function (assert) {
    ...
    const result = map.getMergedGeometry('1', ['1', '2'], '2', ['1', '2'], true);
  ...
  test('getMergedGeometry with geometry reducer and difference should return geoJson feature in EPSG:4326', function (assert) {
    ...
    const toJstsStub1 = sinon.stub(feature1Layer1, 'toJsts');
    const objJsts1 = geometryToJsts(geoJson1Layer1, 10000);
    objJsts1.setSRID(4326);
    toJstsStub1.returns(objJsts1);
    ...
    const toJstsStub2 = sinon.stub(feature2Layer1, 'toJsts');
    const objJsts2 = geometryToJsts(geoJson2Layer1, 10000);
    objJsts2.setSRID(4326);
    toJstsStub2.returns(objJsts2);
    ...
    const toJstsStub3 = sinon.stub(feature1Layer2, 'toJsts');
    const objJsts3 = geometryToJsts(geoJson1Layer2, 10000);
    objJsts3.setSRID(4326);
    toJstsStub3.returns(objJsts3);
    ...
    const toJstsStub4 = sinon.stub(feature2Layer2, 'toJsts');
    const objJsts4 = geometryToJsts(geoJson2Layer2, 10000);
    objJsts4.setSRID(4326);
    toJstsStub4.returns(objJsts4);
    ...
    const result = map.getMergedGeometry('1', ['1', '2'], '2', ['1', '2'], false, 10000);
  ...
  test('getMergedGeometry with geometry reducer and union should return geoJson feature in EPSG:4326', function (assert) {
    ...
    const result = map.getMergedGeometry('1', ['1', '2'], '2', ['1', '2'], true, 10000);
