...
test('createTimeInterval() test', function(assert) {
  ...
  let date = moment(input, timeInfo.dateFormat + timeInfo.timeFormat);
  let [startInterval, endInterval] = createTimeInterval(date, timeInfo.dateFormat);

  assert.equal('2021-01-12T05:30:22.000Z', startInterval);
  assert.equal('2021-01-12T05:30:23.000Z', endInterval);

  let input2 = '2021-01-12T12:20:30.000Z';
  let timeInfo2 = getDateFormatFromString(input2);
  let date2 = moment(input2, timeInfo2.dateFormat + timeInfo2.timeFormat);
