...
module('Unit | Utility | layer to ewkt', function () {
  ...
  test('test method latLngsToCoords for Point', function (assert) {
    ...
    // Act
    const result = latlngToPointEWKT(latlng, crs);
    const resultWithAlt = latlngToPointEWKT(latlngWithAlt, crs);
  ...
  test('test method latlngToPolylineEWKT for LineString', function (assert) {
    // Arrange
    const latlng = [L.latLng(30, 10), L.latLng(10, 30), L.latLng(40, 40)];
    const latlngWithAlt = [L.latLng(30, 10, 20), L.latLng(10, 30, 21), L.latLng(40, 40, 22)];

    // Act
    const result = latlngToPolylineEWKT(latlng, crs);
    const resultWithAlt = latlngToPolylineEWKT(latlngWithAlt, crs);
  ...
  test('test method latlngToPolylineEWKT for MultiLineString', function (assert) {
    ...
    // Act
    const result = latlngToPolylineEWKT(latlng, crs);
    const resultWithAlt = latlngToPolylineEWKT(latlngsWithAlt, crs);
  ...
  test('test method latlngToPolygonEWKT for Polygon without hole', function (assert) {
    ...
    // Act
    const result = latlngToPolygonEWKT(latlng, crs);
    const resultWithAlt = latlngToPolygonEWKT(latlngsWithAlt, crs);
