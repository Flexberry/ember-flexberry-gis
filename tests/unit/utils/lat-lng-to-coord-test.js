...
module('Unit | Utility | lat lng to coord', function () {
  const crs = crsFactory4326.create();
  const precision = 0;

  test('test method latLngToCoords for Point', function (assert) {
    // Arrange
    const latlng = L.latLng(30, 10);
    const latlngWithAlt = L.latLng(30, 10, 20);
    ...
    // Act
    const resCoord = latLngToCoords(latlng, crs, precision);
    const resCoordWithAlt = latLngToCoords(latlngWithAlt, crs, precision);
    const resCoordWithFunc = latLngToCoords(latlng, crs, precision, coordinatesFunction);
    const resCoordWithAltAndFunc = latLngToCoords(latlngWithAlt, crs, precision, coordinatesFunction);
  ...
  test('test method latLngsToCoords for LineString', function (assert) {
    // Arrange
    const latlngs = [L.latLng(30, 10), L.latLng(10, 30), L.latLng(40, 40)];
    const latlngsWithAlt = [L.latLng(30, 10, 20), L.latLng(10, 30, 21), L.latLng(40, 40, 22)];
    const coordinatesFunction = function (coord, altitude) {
      return altitude
        ? new jsts.geom.Coordinate(coord.x, coord.y, altitude)
        : new jsts.geom.Coordinate(coord.x, coord.y);
    ...
    // Act
    const resCoord = latLngsToCoords(latlngs, crs, levelsDeep, closed, precision);
    const resCoordWithAlt = latLngsToCoords(latlngsWithAlt, crs, levelsDeep, closed, precision);
    const resCoordWithFunc = latLngsToCoords(latlngs, crs, levelsDeep, closed, precision, coordinatesFunction);
    const resCoordWithAltAndFunc = latLngsToCoords(latlngsWithAlt, crs, levelsDeep, closed, precision, coordinatesFunction);
