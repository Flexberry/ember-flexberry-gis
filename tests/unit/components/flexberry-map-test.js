...
module('Unit | Component | flexberry map', function (hooks) {
  setupTest(hooks);

  test('it should create leaflet map on didInsertElement', function (assert) {
    const component = this.owner.factoryFor('component:flexberry-map').create();
    this.render();
  ...
  test('test function queryToMap', function (assert) {
    ...
    const querySpy = sinon.stub(leafletMap, 'fire', (st, e) => {
  ...
  test('should compute center from lat/lng', function (assert) {
    ...
    const component = this.owner.factoryFor('component:flexberry-map').create({ lat, lng, });
