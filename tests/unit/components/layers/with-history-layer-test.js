...
moduleForComponent('layers/with-history-layer', 'Unit | Component | layers/with history layer', {
  ...
  beforeEach() {
    ...
    const timetestModel = EmberFlexberryDataModel.extend(testModelMixin);
    ...
    const layerModelWfs = store.createRecord('test-model');
...
test('it return L.WFS on createLayer', function (assert) {
  const component = this.subject(paramWFS);
...
test('it return Odata on createLayer', function (assert) {
  const component = this.subject(paramOdata);
  const done = assert.async(1);
