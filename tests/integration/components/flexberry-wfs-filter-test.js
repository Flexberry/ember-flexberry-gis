...
module('Integration | Component | flexberry wfs filter', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(hbs`{{flexberry-wfs-filter}}`);

    assert.notEqual(this.element.textContent.indexOf('Fields isn\'t loaded'), -1, 'Should contain "Fields isn\'t loaded"');
  });
});
