...
export default IdentifyMapTool.extend({
  ...
  */
  ...
  _renderLayer({ layer }) {
    ...
    let _bufferLayer = L.geoJSON(buf).addTo(this.get('leafletMap'));
    _bufferLayer.addLayer(layer);
    this.set('layer', layer);
    this.set('_bufferLayer', _bufferLayer);
    this.leafletMap.fire('flexberry-map:geomChanged', { wkt: _bufferLayer.getLayers()[0].toEWKT(L.CRS.EPSG4326) });
