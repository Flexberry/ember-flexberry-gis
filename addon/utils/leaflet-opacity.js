...
const maxGeometryOpacity = 0.65;

const maxGeometryFillOpacity = 0.2;
...
*/
const setLeafletLayerOpacity = function ({
  ...
  if (typeof leafletLayer.setOpacity === 'function') {
    ...
    const style = get(leafletLayer, 'options.style') || {};
    if (typeof style === 'function') {
      let opacityFromFunction = 1;
      if (leafletLayer.feature) {
        const styleFromFunction = style(leafletLayer.feature) || {};
  ...
  if (leafletLayer instanceof L.MarkerClusterGroup) {
    setLeafletLayerOpacity({
      leafletLayer: get(leafletLayer, '_featureGroup'), opacity, maxGeomOpacity, maxGeomFillOpacity,
    });
  }
  ...
  if (typeof leafletLayer.eachLayer === 'function') {
    leafletLayer.eachLayer((nestedLeafletLayer) => {
      setLeafletLayerOpacity({
        leafletLayer: nestedLeafletLayer, opacity, maxGeomOpacity, maxGeomFillOpacity,
      });
    });
  }
};
