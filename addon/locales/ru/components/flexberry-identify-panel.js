export default {
  'identify-all': {
    'caption': 'Все слои'
  },
  'identify-all-visible': {
    'caption': 'Все видимые слои'
  },
  'identify-top-visible': {
    'caption': 'Верхний видимый слой'
  },
  'rectangle': {
    'caption': 'Прямоугольник'
  },
  'polygon': {
    'caption': 'Многоугольник'
  },
  'file': {
    'caption': 'Загрузить файл',
    'form-caption-file': 'Файл *',
    'form-caption-sk': 'Система координат',
    'help': '<div class="content">Загрузите файл формата: ' +
    'TAB (в архиве .zip), MIF/MID (в архиве .zip), SHP (в архиве .zip), KML, GEOJSON (JSON), XLS, CSV, XML, GPX, GML. ' +
    'Набор связанных файлов, составляющих файл формата TAB, MIF/MID или SHP, должен быть сохранен в основной директории архива .zip, ' +
    'а не в директориях, созданных внутри архива.</div>',
    'identify': { 'caption': 'Идентифицировать' },
    'create-layer': { 'caption': 'Переход к созданию слоя из файла' },
    'preview': { 'caption': 'Просмотр области' }
  },
  'marker': {
    'caption': 'Точка'
  },
  'polyline': {
    'caption': 'Ломаная'
  },
  'clear': {
    'caption': 'Очистить'
  },
  'buffer': {
    'active-caption': 'Буфер',
    'radius-caption': 'Радиус',
    'units': {
      'meters': 'Метры',
      'kilometers': 'Километры'
    }
  }
};
